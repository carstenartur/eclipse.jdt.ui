...
public class DisableTestAction extends Action {
	...
	private IMethod findTestMethod(IType type, String methodName) throws JavaModelException {
		// Similar to TestMethodFinder.findMethodForParameterizedTest, but operates on IType directly
		...
		astRoot.accept(new ASTVisitor() {
			...
			public boolean visit(MethodDeclaration node) {
				if (node.getName().getIdentifier().equals(method.getElementName())) {
					// Default to JUnit 5 (@Disabled) unless explicitly a JUnit 4 test (@Test from org.junit).
					// If neither annotation is present, JUnit 5 is the safer default for modern projects.
